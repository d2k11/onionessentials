...
public class YellCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String[] strings) {
        ...
        for(Player p : Bukkit.getOnlinePlayers()) {
            p.sendMessage(Log.format("&7<0> " + settings.color + id + " &4» &f" + String.join(" ", strings)));
        }
        Bukkit.getConsoleSender().sendMessage("&7<0> " + Log.format(settings.color + id + " &4» &f" + String.join(" ", strings)));
        return true;
    }
}
